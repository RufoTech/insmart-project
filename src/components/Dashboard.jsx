              ...
              <div className="flex items-center justify-between mb-6">
                ...
                <div className="flex items-center gap-4">
                  <div className="flex items-center gap-6">
                    <button
                      className={`flex items-center gap-2 text-sm ${activeTab === "day" ? "text-blue-600" : "text-gray-500"}`}
                      onClick={() => setActiveTab("day")}
                    >
                      <span
                        className={`w-3 h-3 rounded-full ${activeTab === "day" ? "bg-blue-600" : "bg-gray-300"}`}
                      ></span>
                      Gün
                    </button>
                    <button
                      className={`flex items-center gap-2 text-sm ${activeTab === "week" ? "text-blue-600" : "text-gray-500"}`}
                      onClick={() => setActiveTab("week")}
                    >
                      <span
                        className={`w-3 h-3 rounded-full ${activeTab === "week" ? "bg-blue-600" : "bg-gray-300"}`}
                      ></span>
                      Həftə
                    </button>
                    <button
                      className={`flex items-center gap-2 text-sm ${activeTab === "month" ? "text-blue-600" : "text-gray-500"}`}
                      onClick={() => setActiveTab("month")}
                    >
                      <span
                        className={`w-3 h-3 rounded-full ${activeTab === "month" ? "bg-blue-600" : "bg-gray-300"}`}
                      ></span>
                      Ay
                    </button>
                  </div>
                  ...
                  <BarChart data={barChartData}>
                    ...
                    <Bar dataKey="value" radius={[4, 4, 0, 0]}>
                      {barChartData.map((entry, index) => (
                        <Cell key={`cell-${index}`} fill={index === 2 ? "#2563eb" : "#e5e7eb"} />
                      ))}
                    </Bar>
                  </BarChart>
                </ResponsiveContainer>
